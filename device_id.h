// device_id.h — per-MAC unique hostname for this sensor station.
//
// All units flash the same firmware, so a static "cyd-hydro" hostname
// would collide on a LAN with multiple CYDs. We derive a 4-hex suffix
// from the lower 16 bits of the MAC address — the same convention
// hydro-dash and cores3-hydro use — yielding e.g. "cyd-hydro-a3f2".
//
...
//   - display in the hero footer (so the user can identify which CYD
//     is which when several are deployed)

#pragma once

// Returns a stable per-device hostname like "cyd-hydro-a3f2".
