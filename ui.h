...
//
// The UI never does I/O. It reads from the local sensor atomics in
// state.h and renders. Touch events from touch.cpp call ui_handle_touch().
//
// Single device, so no per-device drilldown — UI_SCREEN_DETAIL from
// the hydro-dash codebase isn't ported.
